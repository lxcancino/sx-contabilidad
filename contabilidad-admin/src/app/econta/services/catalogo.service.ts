...
export class CatalogoService {
  ...
  list(max = 50, sort = 'dateCreated', order = 'desc'): Observable<Catalogo[]> {
  ...
  generar(ejercicio: number, mes: number): Observable<Catalogo> {
    return this.http
      .post<Catalogo>(this.apiUrl, { ejercicio, mes })
      .pipe(catchError((error: any) => throwError(error)));
  }
